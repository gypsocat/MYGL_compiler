...
using bool8vec = std::vector<MTB::bool8_t>;
...
namespace MTB {
    ...
    public:
        ...
        uint32_t cached_id_number() const {
            return _cur_max_id - 2;
        }
        uint32_t get_allocated_id_number() const {
            return _allocated_nelems;
        }
        uint32_t unallocated_id_number() const {
            return cached_id_number() - _allocated_nelems;
        }
    private:
        /** @struct Entry
         *  @brief  ID索引 */
